...
프로젝트 관련 예상질문 
질문 1: 음식점 및 맛집 데이터가 부족한데 어떻게 문제를 해결하셨나요 ?
답변 1: 공공데이터 포털 사이트에서 맛집 데이터를 찾을 수 있었습니다. 
저희가 찾은 맛집 데이터는 서울특별시 광진구 47건(2021.01.29기준), 용산구 맛집 314건,
 인천광역시 서구 맛집 32건(2020.08.01기준),
충청북도 괴산군 맛집 39건(2019.09.05기준) , 공공데이터 맛집 광장 데이터 43건해서
총 475건의 데이터를 수집해서 MYSQL DB에 데이터를 저장해놓았습니다.
저희가 수집한 공공데이터를 통해서 가게 주소, 가게명, 카테고리(한식 등), 위도, 경도 등의 정보를 알 수 있습니다
또한 내맛집지도에서 가게 등록하기 버튼을 누르면 가게명, 가게 주소, 카테고리(한식 등), 위도, 경도
정보가 자동으로 DB에 저장이 되기 때문에 가게 정보 데이터를 수집하는데 문제가 없다고 생각합니다.


질문 2: MYSQL DB를 사용하신 이유가 무엇인가요 ?
답변 2: 저희 프로젝트는 테이블이 많기 때문에 관계형 데이터베이스인 MYSQL를 사용해서 ERD(ER-Diagram)을 만들어야 했고,
MYSQL Workbench에는 ERD를 자동으로 스키마(데이터베이스)와 테이블을 만들어주었기 때문에 편의성 측면에서 사용하게 되었습니다.
또한 다수의 데이터를 저장하고 관리하는데 있어서 MYSQL Workbench를 사용하면 수백 건의 데이터를 한눈에 보기 편해서 수집된 데이터들을
효율적으로 관리할 수 있었습니다.

질문 3: JWT를 사용하신 이유는 무엇인가요?
답변 3: 
- 서버시스템측에서 더 이상 유저(USER)의 정보를 유지하지 않고, 유저가 회원 인증을 하게 될 때 토큰을 발급해줌으로서 유저가 자기 자신임을 인증 할 수 있게 해주기 위함입니다.
- 토큰에는 유저의 정보(username, userID, user_num 등..)와 토큰의 유효기간을 담을 수 있어서 토큰의 유저 정보를 통해 서버에서 검증을 통해 유저(USER) 인증을 할 수 있습니다
- 저희 프로젝트를 사용하시려면 백엔드 2개, 프론트엔드 1개의 서버가 필요합니다. 이처럼 다수의 서버를 사용할 때, 토큰을 사용함으로서, 서버를 확장하게 될 때에 매우 용이해지게 됩니다. 
- 즉, 서버 시스템이 분산되어있어도, 유저(USER)는 같은 토큰으로 서버에 요청을 하면 되고, 서버는 그저 그 토큰이 위조되지 않았는지만 검증을 하고 데이터베이스 조회도 할 필요 없이 바로 사용자임을 신뢰하고 처리할 수 있기 때문입니다.
- 추가적으로, 토큰을 사용하면 로그인시에 페이스북, 구글과 같은 플랫폼에서 토큰을 발급해주면 토큰을 기반으로 권한을 공유하게 되어 유저가 서비스를 사용할 수 있게 됩니다.


질문 4: JWT 토큰을 쿠키에 저장한 이유는 무엇인가요?
답변 4:
- 서버가 토큰을 발급해주면, 브라우저에서 사용자 / 서버간에 토큰이 전달되는 방식은 크게 두가지로 나뉩니다
- 첫번째 방법으로는 토큰을 웹 스토리지 (localStorage 혹은 sessionStorage)에 넣고, 그 다음부터 웹 요청을 할 때마다 HTTP 헤더 값에 토큰을 넣어서 요청하는 방법이 있습니다.
- 두번째 방법으로는 토큰을 쿠키에 넣는 방법이 있습니다.
- 첫번째 방법은 하나의 도메인에서 토큰 활용이 제한되어있지 않다는 장점이 있지만, XSS 해킹 공격을 통하여 해커의 악성스크립트에 노출이 되는 경우 매우 쉽게 토큰이 탈취 될 수 있습니다. 
- localStorage에 접근하면 바로 토큰에 접근할 수 있기 때문입니다.
- 그래서 저희 프로젝트는 쿠키에 토큰을 담았습니다. 하지만 쿠키에 토큰을 담으면 CSRF에 취약해지지만, CSRF는 보안에 신경을 쓰면 차단할 수 있다고 합니다.
